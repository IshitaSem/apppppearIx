...
from database import wardrobe_collection, serialize_list
...
def get_user_items(user_id: str) -> List[Dict]:
    """FIXED: Use .find() instead of direct collection iteration"""
    cursor = wardrobe_collection.find({"user_id": user_id})
    items = serialize_list(cursor)  # Converts ObjectId and ensures JSON-safe
    for item in items:
